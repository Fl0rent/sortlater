...
import { Bookmark, Archive, List, Grid, ChevronLeft, ChevronRight, ExternalLink, Zap } from 'lucide-react';
...
export const HomePage: React.FC<HomePageProps> = ({ 
        ...
        <button
          ...
          className={`p-2 rounded-lg transition-all duration-200 ${
            currentPage === 1
              ...
              : isDark ? 'bg-gray-900 text-green-400 hover:bg-gray-800 border border-green-500/20' : 'bg-white text-gray-700 hover:bg-gray-100 shadow-md'
          }`}
          title="Previous page"
        >
          ...
          {pages.map(page => (
            <button
              ...
              className={`px-3 py-2 rounded-lg font-medium transition-all duration-200 ${
                currentPage === page
                  ? isDark ? 'bg-green-500 text-black shadow-lg shadow-green-500/25' : 'bg-blue-500 text-white shadow-lg'
                  : isDark ? 'bg-gray-900 text-gray-300 hover:bg-gray-800 border border-green-500/20' : 'bg-white text-gray-600 hover:bg-gray-100 shadow-md'
              }`}
            >
              {page}
            </button>
  ...
  return (
    <div className="container mx-auto px-4 max-w-4xl">
      ...
      {/* Links List */}
      <div className={`mb-8 ${displayMode === 'simple' ? 'space-y-2' : 'space-y-4'}`}>
        {filteredLinks.length === 0 ? (
          ...
          <>
            ...
            {/* Pagination Info */}
            {totalPages > 1 && (
              <div className={`text-center text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} mt-4`}>
      ...
      <div className="flex items-center justify-center space-x-8 mb-6 mt-8">
        <div className="text-center">
          <div className={`text-2xl font-bold ${isDark ? 'text-green-400' : 'text-blue-600'}`}>{activeLinks.length}</div>
          <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Active</div>
        </div>
        <div className="text-center">
          <div className={`text-2xl font-bold ${isDark ? 'text-green-300' : 'text-purple-600'}`}>{archivedLinks.length}</div>
          <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Archived</div>
        </div>
        <div className="text-center">
          <div className={`text-2xl font-bold ${isDark ? 'text-green-200' : 'text-pink-600'}`}>{links.length}</div>
        ...
        <button
          ...
          className={`flex items-center space-x-2 px-6 py-3 rounded-lg font-medium transition-all duration-200 ${
            currentView === 'archived'
              ? isDark ? 'bg-green-500 text-black shadow-lg shadow-green-500/25' : 'bg-purple-500 text-white shadow-lg'
              : isDark ? 'bg-gray-900 text-gray-300 hover:bg-gray-800 border border-green-500/20' : 'bg-white text-gray-600 hover:bg-gray-100 shadow-md'
          }`}
        >
      ...
      <div className="flex items-center justify-center space-x-1 mb-16">
        <button
          ...
          className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
            displayMode === 'simple'
              ? isDark ? 'bg-green-500 text-black shadow-lg shadow-green-500/25' : 'bg-green-500 text-white shadow-lg'
              : isDark ? 'bg-gray-900 text-gray-300 hover:bg-gray-800 border border-green-500/20' : 'bg-white text-gray-600 hover:bg-gray-100 shadow-md'
          }`}
      ...
      {/* Footer */}
      <footer className={`text-center ${isDark ? 'text-gray-500' : 'text-gray-500'} text-sm mt-16 pb-8`}>
        <p>SortLater - Your personal read-it-later application</p>
        <p className="text-xs mt-1">
          Press <kbd className={`px-1 py-0.5 rounded text-xs ${isDark ? 'bg-gray-800 text-green-400 border border-green-500/20' : 'bg-gray-200'}`}>?</kbd> for keyboard shortcuts
        </p>
      </footer>
    </div>
  );
